...
export class StdioLspClient {
  ...
  constructor(private readonly opts: StdioLspClientOptions) {}
  ...
  async start(): Promise<void> {
    ...
    this.child.stderr.on("data", (chunk: Buffer) => {
      ...
      if (msg) this.opts.onError?.(`[${this.opts.serverId}] ${msg.slice(0, 400)}`);
    });
    this.child.on("exit", (code) => {
      this.initialized = false;
      for (const [, p] of this.pending) {
        p.reject(new Error(`LSP server ${this.opts.serverId} exited (${code})`));
      }
      this.pending.clear();
  ...
  private onMessage(body: string): void {
    ...
    if (msg.method === "textDocument/publishDiagnostics") {
      ...
      const diagnostics: LspDiagnostic[] = (params.diagnostics ?? []).map((d) => ({
        ...
        source: d.source ?? this.opts.serverId,
        code: d.code,
      }));
