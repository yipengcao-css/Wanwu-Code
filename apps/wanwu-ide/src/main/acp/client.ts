...
/**
 * Minimal newline-delimited JSON-RPC ACP client. Mirrors the message shapes
 * produced by `packages/wanwu-cli` (mock + grok bridge).
 */
export class AcpClient extends EventEmitter {
  ...
  constructor(private readonly child: ChildProcessWithoutNullStreams) {
    ...
    child.on("exit", (code) => this.emit("exit", code ?? 0));
  ...
  private onLine(line: string): void {
    ...
    if (typeof msg.method === "string") {
      ...
      if (tool) this.emit("tool", tool);
      const edit = extractEdit(msg.params);
      if (edit) this.emit("edit", edit);
  ...
  async initialize(): Promise<unknown> {
    return this.request("initialize", {
      ...
      clientInfo: { name: "wanwu-ide", version: "0.1.0" },
    });
  }
...
function extractTool(params: unknown): AcpToolUpdate | undefined {
  if (!params || typeof params !== "object") return undefined;
  const p = params as Record<string, unknown>;
  const update = p.update as Record<string, unknown> | undefined;
