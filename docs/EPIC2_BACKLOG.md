...
| 20 | 云端容器执行 + diff apply | **完成（#26）** |
| 21 | 权限规则文件（allow/ask/deny） | **完成（#27）** |
| 22 | TUI 升级（主题 / 状态栏） | **完成（#28）** |
| 23 | 通用 Verify（检测项目类型） | **完成（#29）** |
| 24 | 子代理 worktree 隔离 | **完成（#30）** |
| 25 | MCP 对话式配置 | **完成（#31）** |
| 26 | IDE 宿主覆盖（Zed / JetBrains 文档） | **完成（#32）** |
| 27 | TUI 多 pane（布局 / 状态栏 / 工具面板） | **完成（#33）** |
| 28 | 真云端快照 + 编排 | **完成（#34）** |
| 29 | 多模态输入 | **完成（#35）** |
| 30 | 丰富 hooks 生命周期 | **完成（#36）** |
| 31 | IDE 深度集成（VS Code 内联 / JetBrains / Zed scaffold） | **完成（#37）** |
| 32 | Cursor 对标审计 | **完成（#38）** |
| 33 | Cursor 对标 A–E | **完成（#39–#56）** |

## 领取方式

表内 1–33 与 2026-09-18/21 体感项均已落地或跳过。余下见 `docs/CURSOR_PARITY_2026-09-15.md` 文首「仍未做」。不要按该文档 §1–§3 旧空缺表施工。
